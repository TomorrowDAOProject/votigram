...
vi.mock('../Upload', () => ({
  default: ({ onFinish }: IUploadProps) => (
    <input type="file" data-testid="upload-btn" onChange={() => onFinish?.('new-icon')} />
  ),
}));
...
describe('InputGroup Component', () => {
  ...
  beforeEach(() => {
    handleChange = vi.fn((options) => {
      console.log('handleChange called with:', options);
    });
  });
  ...
  it('adds a new option when "Add New Option" button is clicked', () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(expect.arrayContaining([{ id: expect.any(Number), title: '' }]));
  });
